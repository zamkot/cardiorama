...
void Hrv1Module::runHrv1(){
	...
	vec timeDomainResults = timeDomain(F, data, tk);s

    results = {F,P,timeDomainResults};
    validateResults();
...
Hrv1Module::arma::vec timeDomain(arma::vec& F, arma::vec& data, arma::vec& tk){
	...
	double meanRR  = mean(data) * 1000;
	
	// standard deviation of RR intervals
	double stdRR = stddev(data);
	...
	double rmsRR = sqrt((pow((data.n_elem-1), -1))*sumRms);
	...
	double pNN50 = double(sumNN50)/(double(data.n_elem-1))*100;
	...
	double sdann = stddev(meanSdann);
	double sdanni = mean(stdSdanni);
	...
	double sdsd = stddev(signalSdsd);

    // EDIT return -> returning in std or arma, which one to choose
    return;
}
