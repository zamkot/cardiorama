...
void Hrv2Module::runHrv2() {
    ...
    double sampling_frequency = 360;
    vec rr = createRRVector(rpeaksmodule_output.rpeaks, sampling_frequency);
    vec rr_filtered = rrFiltering(rr);
 
    // histogram values
    double space_between_bin = (max(rr_filtered)-min(rr_filtered))/config.num_of_bins;
    vec bin_centers = linspace<vec>(min(rr_filtered)+space_between_bin/2,max(rr_filtered)-
                      space_between_bin/2,config.num_of_bins);
    ...
    vec triangular = zeros<vec>(config.num_of_bins);
    double temp = 0;
    double error = 0;
    uword N = 0;
    uword M = 0;
    ...
    for(uword N = 0; N<index_max(hist_values); ++N){
        for(uword M=config.num_of_bins-1; M>index_max(hist_values); --M){
            triangular = createTriangularFunction(N,M,index_max(hist_values), max(hist_values),
                         bin_centers, config.num_of_bins);
            temp = errorCalculation(hist_values,triangular);
            if((N==0 && M==config.num_of_bins-1) || temp < error){
                error = temp;
                N_min = N;
                M_min = M;
            }
...
arma::vec Hrv2Module::createRRVector(std::vector<int> &rpeaks_output, double sampling_frequency){
   
   using namespace arma;
   using namespace std;

   vec rpeaks_index = zeros<vec>(rpeaks_output.size()); 
   
   for(int i = 0; i < rpeaks_output.size(); i++){
        rpeaks_index(i) = rpeaks_output[i];
    }
...
arma::vec Hrv2Module::rrFiltering (arma::vec& rr){
...
arma::vec Hrv2Module::createTriangularFunction(arma::uword N, arma::uword M, arma::uword index_of_maximum, 
...
double Hrv2Module::errorCalculation(arma::uvec& hist_values,arma::vec& triangular_values){
