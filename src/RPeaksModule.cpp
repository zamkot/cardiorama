...
const double RPeaksModule::SAFETY_COEFFICIENT = 0.8;
const unsigned short RPeaksModule::MAX_BPM = 220;
const double RPeaksModule::THRESHOLD_DIVISOR = 20;
const unsigned int RPeaksModule::PT_LP_M = 9;
const unsigned int RPeaksModule::PT_HP_M = 56;
const unsigned int RPeaksModule::PT_MW_N = 58;


RPeaksModule::RPeaksModule(EcgBaselineModule& ecgBaselineModule): ecgBaselineModule{ ecgBaselineModule }
{
}

void RPeaksModule::configure(RPeaksAlgorithm rpeaksAlgorithm, double frequency)
{
...
{
    ecgBaselineModule.runEcgBaseline();
    EcgBaselineData ecgBaselineModuleInput = ecgBaselineModule.getResults();

    this->filteredSignal = std::vector<double>(ecgBaselineModuleInput.samples.begin(), ecgBaselineModuleInput.samples.end());
    this->ecgSignal = std::vector<double>(ecgBaselineModuleInput.samples.begin(), ecgBaselineModuleInput.samples.end());

    if(rpeaksAlgorithm == HILBERT) hilbert();
    if(rpeaksAlgorithm == PAN_TOMPKINS) panTompkins();

    auto rpeaksModuleOutput = getPeaksIndex();
...
void RPeaksModule::fft(const std::vector<double> & x, VectorComplexDouble & result) {
...
void RPeaksModule::ifft(const VectorComplexDouble & x, VectorComplexDouble & result) {
    int N = x.size();
...
void RPeaksModule::fftHilbert(const std::vector<double> & x, std::vector<double> & result) {
    int N = x.size();
...
void RPeaksModule::fftHilbertWindowed(const std::vector<double> & x, std::vector<double> & result, double window_length, double window_overlap) {
...
void RPeaksModule::setHilbertRPeaks(const VectorPairDoubleUnsignedInt & peaks, double dt) {
...
void RPeaksModule::hilbert(void) {
...
double RPeaksModule::PTLowPassFilter(double data, bool reset = false)
{
...
double RPeaksModule::PTHighPassFilter(double data, bool reset = false)
{
...
double RPeaksModule::PTDerivative (double data, bool reset = false)
{
    double y;
...
double RPeaksModule::PTSquaringFunction(double data)
{
    return data*data;
}

double RPeaksModule::PTMovingWindowIntegral(double data, bool reset = false)
{
...
PairDoubleUnsignedInt RPeaksModule::Maximum(int begin, int end, std::vector<double> & values) {
...
PairDoubleUnsignedInt RPeaksModule::Peak200MS(int & index, std::vector<double>  & signal) {
...
int RPeaksModule::QRSCheck(PairDoubleUnsignedInt rpCandidate, PairDoubleUnsignedInt rpLast, double threshold, std::vector<double> & slope) {
...
void RPeaksModule::PanTompkinsDo(std::vector<double> & signal, VectorPairDoubleUnsignedInt & result) {
...
void RPeaksModule::PanTompkinsSetPeaks(VectorPairDoubleUnsignedInt & filter, VectorPairDoubleUnsignedInt & movingWindow) {
...
void RPeaksModule::panTompkins(void) {
    this->indexVector.clear();
    this->itVect.clear();
    int N = this->filteredSignal.size();
...
const R_peaksIterVector & RPeaksModule::getPeaksIter(void) {
    return this->itVect;
}

const std::vector<double> & RPeaksModule::getPeaksIndex(void) {
    return this->indexVector;
}

const R_peaksIter RPeaksModule::ecgBegin(void) {
