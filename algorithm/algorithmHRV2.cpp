...
/**/
arma::vec createRRVector(arma::vec& rpeaks_index, double sample_frequency){
   using namespace arma;
...
int main(){
    ...
    /*STDVEC TO ARMAVEC 
    vec my_armavector(my_stdvector);
    */
    ...
    cout << "SD1 = " << SD1 <<" SD2 = " << SD2 << "\n";
    double centroid_x = mean(x_axis);
    double centroid_y = mean(y_axis);

    /* ARMAVEC TO STDVEC
    vector<double> my_stdvector_rrpeaks = conv_to< stdvec >::from(my_armavector); 
    */
